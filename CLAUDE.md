...
| Layer         | Choice                     | Why                                        |
|---------------|----------------------------|--------------------------------------------|
| Framework     | Next.js 16 App Router + TS (React 19) | Same repo for frontend and API, serverless |
...
- Icon set: `icon.svg` (source), plus 192×192, 512×512, and a maskable
  512×512 (80% safe-zone padding on the `#6366f1` background) rasterized as
  PNGs. Generated by `scripts/generate-icons.mjs` (`sharp` devDependency) —
  run `npm run icons:generate` after editing `public/icon.svg` and commit the
  regenerated PNGs (they're checked in, not built at deploy time).
...
# Typecheck + lint (ESLint 9 flat config in eslint.config.mjs — replaces the
# `next lint` command Next 16 removed; see that file's header comments for scope)
npx tsc --noEmit
npm run lint
...
# Canonical local gate before pushing — tsx chain + vitest, see below
npm run test:all
...
# Regenerate PWA icons after editing public/icon.svg (§9); commit the PNGs
...
**Runtime deps as of the Next 16 / React 19 upgrade:** `next@^16.2.10`,
`react@^19.2.7`, `react-dom@^19.2.7`, `@supabase/ssr@^0.6.1`. `sharp` (icon
generation) and `eslint`/`eslint-config-next` (lint) are devDependencies
only — no new runtime deps beyond what Next 16 itself requires.
